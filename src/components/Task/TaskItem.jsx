...
const TaskItem = ({ id, text, completed }) => {
  const { onRemoveTask, activeItem, onEditTask, onChangeChecked } =
    React.useContext(AppContext);

  const onChangeCheckbox = (e) => {
    onChangeChecked(activeItem?.id, id, e.target.checked);
  };
  return (
    <div key={id} className="tasks__items">
      ...
      <p style={{ flex: 1, marginLeft: 10 }}>{text}</p>
      <div className="tasks__items__instrument-panel">
        <svg
          onClick={() => onEditTask(activeItem.id, { id, text })}
          width="15"
          height="15"
          viewBox="0 0 15 15"
          fill="none"
        ...
        <svg
          onClick={() => onRemoveTask(activeItem?.id, id)}
          width="10"
          height="10"
          viewBox="0 0 10 10"
          fill="none"
